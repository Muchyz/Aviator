...
/* CANVAS */
.canvas{position:relative;height:220px;background:linear-gradient(180deg,#040710 0%,#07090f 100%);overflow:hidden;}
.canvas::before{content:'';position:absolute;inset:0;background-image:linear-gradient(rgba(59,130,246,0.04) 1px,transparent 1px),linear-gradient(90deg,rgba(59,130,246,0.04) 1px,transparent 1px);background-size:48px 48px;}
.csvg{position:absolute;inset:0;width:100%;height:100%}
.plane-el{position:absolute;pointer-events:none;filter:drop-shadow(0 0 8px rgba(245,158,11,0.7));}
...
.mult-num{font-family:'JetBrains Mono',monospace;font-size:54px;font-weight:700;line-height:1;letter-spacing:-2px;transition:color 0.2s;}
.mult-num.waiting{color:var(--text3)}
.mult-num.flying{color:var(--amber)}
.mult-num.flying.hi5{color:#f97316}
.mult-num.flying.hi10{color:var(--purple);animation:bigPulse 0.3s ease infinite}
.mult-num.crashed{color:var(--red);animation:shake 0.35s ease}
...
.mult-label.flying{color:var(--amber)}
                ...
                <svg className="csvg" viewBox="0 0 420 280" preserveAspectRatio="none">
                  ...
                  {gs === "flying" && svgPath && (
                    <>
                      ...
                      <path d={svgPath} fill="none" stroke="rgba(245,158,11,0.7)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                    </>
                  )}
                  {gs === "crashed" && svgPath && (
                    <>
                      <path d={svgPath + ` L ${(planePos.x / 100) * 420} 280 L ${(8 / 100) * 420} 280 Z`} fill="url(#crGrad)" />
                      <path d={svgPath} fill="none" stroke="rgba(244,63,94,0.5)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                    </>
                  )}
                </svg>
