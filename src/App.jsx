...
.dd-phone{font-size:10px;color:var(--t2);font-family:'JetBrains Mono',monospace;margin-top:1px}
...
.ferr-inline{font-size:10px;color:var(--red);margin-top:4px;}
...
.pw-eye{position:absolute;right:13px;top:50%;transform:translateY(-50%);background:none;border:none;color:var(--t2);cursor:pointer;font-size:16px;padding:4px;line-height:1;}
.otp-grid{display:flex;gap:8px;justify-content:center;margin:16px 0}
.otp-box{width:48px;height:56px;background:var(--s);border:1px solid var(--b2);border-radius:10px;color:var(--t);font-family:'JetBrains Mono',monospace;font-size:22px;font-weight:700;text-align:center;outline:none;-webkit-appearance:none;transition:border-color 0.18s;}
.otp-box:focus{border-color:var(--gold);box-shadow:0 0 0 3px rgba(246,195,71,0.07)}
...
.plane{position:absolute;filter:drop-shadow(0 0 9px rgba(246,195,71,0.7));pointer-events:none;}
...
.mult-num{font-family:'Sora',sans-serif;font-size:52px;font-weight:800;line-height:1;transition:color 0.2s;}
...
.cbar{display:flex;align-items:center;gap:5px;padding:7px 14px;border-bottom:1px solid rgba(255,255,255,0.04);overflow-x:auto;}
...
.step-btn{width:44px;height:44px;border-radius:9px;border:1px solid var(--b2);background:var(--s);color:var(--t);font-size:22px;font-weight:300;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:all 0.15s;}
.step-btn:hover{border-color:var(--gold);color:var(--gold)}
.step-btn:disabled{opacity:0.35;cursor:not-allowed}
.step-val{flex:1;background:var(--s);border:1px solid var(--b2);border-radius:9px;padding:10px 12px;color:var(--t);font-family:'JetBrains Mono',monospace;font-size:18px;font-weight:700;text-align:center;outline:none;-webkit-appearance:none;}
.step-val:disabled{opacity:0.4}
.qgrid{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:14px}
.qgbtn{padding:9px 0;background:var(--s);border:1px solid var(--b);border-radius:8px;color:var(--t2);font-family:'JetBrains Mono',monospace;font-size:13px;font-weight:600;cursor:pointer;transition:all 0.15s;text-align:center;}
.qgbtn:hover:not(:disabled){border-color:var(--gold);color:var(--gold)}
.qgbtn:disabled{opacity:0.35;cursor:not-allowed}
.bet-big{width:100%;padding:16px;border-radius:12px;border:none;background:#22c55e;color:#fff;font-family:'Sora',sans-serif;font-size:18px;font-weight:800;cursor:pointer;transition:all 0.18s;letter-spacing:0.2px;line-height:1.3;margin-bottom:12px;}
...
.aco-input{width:58px;background:var(--s);border:1px solid var(--b2);border-radius:7px;padding:5px 7px;color:var(--t);font-family:'JetBrains Mono',monospace;font-size:13px;font-weight:700;text-align:center;outline:none;}
...
.prow.cashed{background:rgba(14,207,158,0.06)}
...
.wmini{padding:12px}
.wm-bal{background:linear-gradient(135deg,#0a1828,#0f2040);border:1px solid var(--b);border-radius:11px;padding:12px;margin-bottom:10px;}
...
.chat-msg{font-size:11px;line-height:1.45;}
...
.chat-send{padding:7px 12px;border-radius:8px;border:none;background:var(--gold);color:#000;font-size:12px;font-weight:700;cursor:pointer;}
...
.big-bal{background:linear-gradient(135deg,#091624,#0e1e38);border:1px solid var(--b);border-radius:12px;padding:16px;margin-bottom:16px;}
...
.hist-row{display:flex;align-items:center;justify-content:space-between;padding:12px 18px;border-bottom:1px solid rgba(255,255,255,0.04);}
...
.lb-row{display:flex;align-items:center;gap:10px;padding:11px 16px;border-bottom:1px solid rgba(255,255,255,0.04);}
...
@media(min-width:600px){
  ...
  .toast{bottom:20px;left:auto;right:16px;transform:none;width:auto;max-width:300px;}
}
...
function PhoneInput({value,onChange,placeholder="7XX XXX XXX"}){
  return(
    <div className="phone-wrap">
      <div className="phone-flag">🇰🇪 +254</div>
      <input className="phone-input" placeholder={placeholder}
    ...
    <Modal onClose={onClose}>
      ...
      <div className="mbody">
        ...
        <div className="fg"><label className="flbl">Password</label>
          <PwInput placeholder="••••••••" value={pass} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()}/></div>
  ...
  return(
    <Modal onClose={onClose}>
      ...
      <div className="mbody">
        {err&&<div className="ferr">{err}</div>}
        <div className="frow">
          <div className="fg"><label className="flbl">First Name</label>
            <input className={`finput ${errs.fn?"err-field":""}`} placeholder="John" value={f.fn}
              onChange={e=>{set("fn")(e.target.value);setErrs(p=>({...p,fn:""}))}}/>
            {errs.fn&&<div className="ferr-inline">⚠ {errs.fn}</div>}</div>
          <div className="fg"><label className="flbl">Last Name</label>
            <input className={`finput ${errs.ln?"err-field":""}`} placeholder="Kamau" value={f.ln}
              onChange={e=>{set("ln")(e.target.value);setErrs(p=>({...p,ln:""}))}}/>
            {errs.ln&&<div className="ferr-inline">⚠ {errs.ln}</div>}</div>
        </div>
        <div className="fg"><label className="flbl">M-Pesa Number</label>
          <PhoneInput value={f.phone} onChange={v=>{set("phone")(v);setErrs(p=>({...p,phone:""}))}}/>
          {errs.phone&&<div className="ferr-inline">⚠ {errs.phone}</div>}</div>
        <div className="frow">
          <div className="fg"><label className="flbl">Password</label>
            <PwInput placeholder="Min 6 chars" value={f.pass} onChange={e=>{set("pass")(e.target.value);setErrs(p=>({...p,pass:""}))}}/>
            {errs.pass&&<div className="ferr-inline">⚠ {errs.pass}</div>}</div>
          <div className="fg"><label className="flbl">Confirm</label>
            <PwInput placeholder="Repeat" value={f.confirm} onChange={e=>{set("confirm")(e.target.value);setErrs(p=>({...p,confirm:""}))}}/>
            {errs.confirm&&<div className="ferr-inline">⚠ {errs.confirm}</div>}</div>
        </div>
...
function RecoverModal({onClose,goLogin,onLogin}){
  ...
  const sendOTP=async()=>{
    if(phone.length<12){setErr("Enter a valid phone number.");return;}
    setLoading(true);setErr("");
    try{
  ...
  const handleOtpChange=(i,v)=>{
    ...
    newOtp[i]=digits[0];
    setOtp(newOtp);
    if(i<5&&digits)otpRefs.current[i+1]?.focus();
  };
  ...
  const verifyOTP=async()=>{
    ...
    try{
      const res=await fetch(`${API}/auth/verify-otp`,{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({phone,code,newPassword:newPass}),
      });
      const data=await res.json();
      if(!res.ok){setErr(data.error||"Verification failed");setLoading(false);return;}
      localStorage.setItem("avipesa_token",data.token);
  ...
  return(
    <Modal onClose={onClose}>
      <div className="mhead">
        <div>
          <div className="mtitle">{step===0?"Reset Password 🔑":"Enter OTP 📱"}</div>
          <div className="msub">{step===0?"We'll send an OTP to your number":"Check your SMS for the 6-digit code"}</div>
        </div>
        <button className="mclose" onClick={onClose}>×</button>
      </div>
      <div className="mbody">
        ...
        {step===0?(
          <>
            <div className="fg"><label className="flbl">M-Pesa Number</label>
              <PhoneInput value={phone} onChange={v=>{setPhone(v);setErr("");}}/></div>
            <div className="fhint" style={{marginBottom:14}}>An OTP will be sent to this number via SMS.</div>
            <button className="btn-primary" onClick={sendOTP} disabled={loading||phone.length<12}>
          ...
          <>
            <div style={{textAlign:"center",marginBottom:8,fontSize:13,color:"var(--t2)"}}>
              OTP sent to +{phone}
            </div>
            ...
            <div className="fg"><label className="flbl">New Password</label>
              <PwInput placeholder="Min 6 characters" value={newPass} onChange={e=>setNewPass(e.target.value)}/></div>
            <div className="fg"><label className="flbl">Confirm Password</label>
              <PwInput placeholder="Repeat password" value={confirm} onChange={e=>setConfirm(e.target.value)}/></div>
            <button className="btn-primary" onClick={verifyOTP} disabled={loading}>
              {loading?"Verifying...":"Reset Password"}
            </button>
            <div className="ffoot">
              <button className="flink" onClick={()=>{setStep(0);setErr("");setOtp(["","","","","",""]);}}> Resend OTP</button>
            </div>
          </>
        )}
      </div>
    ...
    <Modal onClose={step===0?onClose:()=>{}}>
      ...
      <div className="mbody">
        {step===0?(<>
          ...
          <div style={{background:"rgba(246,195,71,0.06)",border:"1px solid rgba(246,195,71,0.2)",borderRadius:10,padding:14,marginBottom:14}}>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:8}}><span style={{color:"var(--t2)"}}>M-Pesa Number</span><span style={{fontFamily:"monospace"}}>+{phone}</span></div>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:8}}><span style={{color:"var(--t2)"}}>Amount</span><span>{fKES(amt)}</span></div>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:13,borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:8,fontWeight:700}}><span style={{color:"var(--t2)"}}>You receive</span><span style={{color:"var(--green)"}}>{fKES(amt)}</span></div>
          </div>
...
function BetPanel({gs,user,hasBet,cashedOut,betAmt,setBetAmt,autoCO,setAutoCO,onBet,onCashout,onLogin,md}){
  ...
  const BigBtn=()=>{
    ...
    if(gs==="flying"&&hasBet&&!cashedOut)return(
      <button className="bet-big cashout" onClick={onCashout}>CASH OUT ×{md}</button>
    );
    ...
    return<button className="bet-big crashed-btn" disabled>Waiting...</button>;
  };
  return(
    <div className="bpanel">
      ...
      {bpTab==="auto"&&(<>
        <div style={{background:"var(--s)",border:"1px solid var(--b)",borderRadius:10,padding:12,marginBottom:14}}>
          <div style={{marginBottom:10}}>
            <span style={{fontSize:10,fontWeight:700,letterSpacing:1,textTransform:"uppercase",color:"var(--t2)",marginBottom:5,display:"block"}}>Bet Amount (KES)</span>
            <input style={{width:"100%",background:"var(--card)",border:"1px solid var(--b2)",borderRadius:8,padding:"8px 10px",color:"var(--t)",fontFamily:"'JetBrains Mono',monospace",fontSize:14,fontWeight:700,outline:"none"}} type="number" value={betAmt} onChange={e=>setBetAmt(e.target.value)}/>
          </div>
          <div>
            <span style={{fontSize:10,fontWeight:700,letterSpacing:1,textTransform:"uppercase",color:"var(--t2)",marginBottom:5,display:"block"}}>Auto Cash Out ×</span>
            <input style={{width:"100%",background:"var(--card)",border:"1px solid var(--b2)",borderRadius:8,padding:"8px 10px",color:"var(--t)",fontFamily:"'JetBrains Mono',monospace",fontSize:14,fontWeight:700,outline:"none"}} type="number" value={autoCO} onChange={e=>setAutoCO(e.target.value)} min="1.1" step="0.1"/>
          </div>
        </div>
...
export default function App(){
  ...
  const connectSocket=useCallback((token)=>{
    ...
    socket.on("game:state",(data)=>{setGs(data.state);setMult(data.multiplier||1);mRef.current=data.multiplier||1;setCd(data.countdown||5);setCrashes(data.history||[]);setPlayers(data.bets||[]);gsRef.current=data.state;});
    socket.on("game:waiting",(data)=>{gsRef.current="waiting";setGs("waiting");setMult(1);mRef.current=1;setPlanePos({x:8,y:18});planePosRef.current={x:8,y:18};setPlaneCrashed(false);setPathPts([]);setCashedOut(false);setHasBet(false);betRef.current=false;coRef.current=false;setCrashes(data.history||[]);setPlayers(data.bets||[]);});
    socket.on("game:countdown",(data)=>{setCd(data.countdown);});
    socket.on("game:flying",(data)=>{gsRef.current="flying";setGs("flying");setPlaneCrashed(false);setPathPts([{x:8,y:82}]);setPlayers(data.bets||[]);});
  ...
  useEffect(()=>{
    if(tab==="leaderboard"){
      fetch(`${API}/game/leaderboard`).then(r=>r.ok?r.json():null).then(data=>{if(data)setLeaderboard(data.map(p=>({name:p.name,total:p.total_won,bets:p.total_bets,best:p.best_cashout})));}).catch(()=>{});
    }
  },[tab]);
  ...
  const svgPath=pathPts.length>1?"M "+pathPts.map(p=>`${(p.x/100)*420} ${(p.y/100)*280}`).join(" L "):"";
  ...
  const stars=useRef(Array.from({length:18},(_,i)=>({id:i,x:Math.random()*100,y:Math.random()*100,size:Math.random()*1.5+0.5,op:Math.random()*0.4+0.2,d:(Math.random()*3+2)+"s",delay:(Math.random()*3)+"s"}))).current;
  ...
  return(
    <div className={`root ${lightMode?"light":""}`} onClick={()=>ddOpen&&setDdOpen(false)}>
      ...
      {modal==="recover"&&<RecoverModal onClose={()=>setModal(null)} goLogin={()=>setModal("login")} onLogin={handleLogin}/>}
          ...
          <div>
            <div className="gcard">
              <div className="gtopbar">
                ...
                <div className={`rbadge ${gs}`}>{gs==="waiting"?`Next in ${cd}s`:gs==="crashed"?"CRASHED":"IN PLAY"}</div>
              </div>
              <div className="canvas">
                <div className="stars">{stars.map(s=>(<div key={s.id} className="star" style={{left:`${s.x}%`,top:`${s.y}%`,width:s.size,height:s.size,opacity:s.op,"--d":s.d,"--delay":s.delay,"--op":s.op}}/>))}</div>
                {explodePos&&(<div className="crash-explode" style={{left:`${explodePos.x}%`,bottom:`${explodePos.y}%`,fontSize:32}}>💥</div>)}
                ...
                {gs==="waiting"&&(<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}><CountdownRing cd={cd} total={5}/></div>)}
                {(gs==="flying"||(gs==="crashed"&&!planeCrashed))&&(
                  <div className={`plane ${planeCrashed?"crashed-spin":""}`} style={{left:`${planePos.x}%`,bottom:`${planePos.y}%`}}>
                    <PlaneSVG size={40}/>
                  </div>
                )}
                {gs!=="waiting"&&(
              ...
              <div className="cbar">
                ...
                {crashes.map((v,i)=>(<span key={i} className={`cbadge ${cbCls(v)} ${i===0?"new":""}`}>{Number(v).toFixed(2)}×</span>))}
              </div>
              <BetPanel gs={gs} user={user} hasBet={hasBet} cashedOut={cashedOut} betAmt={betAmt} setBetAmt={setBetAmt} autoCO={autoCO} setAutoCO={setAutoCO} onBet={handleBet} onCashout={doCashout} onLogin={()=>setModal("login")} multRef={mRef} md={md}/>
            </div>
          </div>
          <div className="rcol">
            <div className="rcard">
      ...
      {tab==="history"&&(
        <div className="page wide">
          <div className="pcard">
            {!user?(<Locked title="History Locked" sub="Sign in to view your transaction history." openLogin={openLogin} openRegister={openRegister}/>):(<>
      ...
      {tab==="stats"&&(
        <div className="page">
          <div className="pcard">
            {!user?(<Locked title="Stats Locked" sub="Sign in to see your personal statistics." openLogin={openLogin} openRegister={openRegister}/>):(<>
              ...
              <div className="pcard-body">
                ...
                <div style={{background:"var(--s)",border:"1px solid var(--b)",borderRadius:12,padding:14}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:8}}><span style={{color:"var(--t2)"}}>Phone</span><span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:11}}>+{user.phone}</span></div>
